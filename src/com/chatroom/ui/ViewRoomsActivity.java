		...
		try {
			...
			while(true) {
				...
				if(obj instanceof Response) {
					...
					if(response.getId() == Response.Type.VIEW_ROOMS.ordinal()) {
						break;
					}
				}
			}
			...
			{
				if(response.getContents().length() > 0)
	...
	private void createAndJoinRoom(String rName, boolean create)
	{
		try {
			...
			while(true) {
				...
				if(obj instanceof Response) {
					response = (Response) obj;
					if((create && response.getId() == Request.Type.CREATE_ROOM.ordinal()) || (!create && response.getId() == Request.Type.JOIN_ROOM.ordinal())) {
						break;
					}
				}
			}
